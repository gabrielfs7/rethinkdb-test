...
module.exports.setup = function () {
  rethinkDb.connect({ host: dbConfig.host, port: dbConfig.port }, function (err, connection) {
    ...
    rethinkDb.dbCreate(dbConfig.db).run(connection, function (err, result) {
      if (err) {
        logdebug(
      ...
      for (var tbl in dbConfig.tables) {
        (function (tableName) {
          rethinkDb.db(dbConfig.db).tableCreate(tableName, { primaryKey: dbConfig.tables[tbl] }).run(connection, function (err, result) {
            if (err) {
              logdebug(
...
module.exports.findMessages = function (max_results, callback) {
  onConnect(function (err, connection) {
    rethinkDb.db(dbConfig['db']).table('messages').orderBy(rethinkDb.desc('timestamp')).limit(max_results).run(connection, function (err, cursor) {
      if (err) {
        logerror(
